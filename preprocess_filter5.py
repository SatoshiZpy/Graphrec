...
if args.dataset == 'Ciao':
	...
	click_f = loadmat(workdir + 'Yelp/rating.mat')['rating']
	trust_f = []
...
for s in click_f:
	...
	if label > rate_count:
		rate_count = label
	click_list.append([review_id, uid, iid, label])
...
pos_df = pd.DataFrame(pos_list, columns = ["rid", 'uid', 'iid', 'label'])
filter_pos_list = []
...
for u in tqdm(range(user_count + 1)):
	...
	filter_pos_list.extend([(rid, u, iid, rating) for rid, iid, rating in zip(u_r, u_items, u_ratings)])
...
with open(workdir + args.dataset + '/dataset_filter5.pkl', 'wb') as f:
	...
	pickle.dump(pos_list, f,  pickle.HIGHEST_PROTOCOL)


pos_df = pd.DataFrame(pos_list, columns = ["rid", 'uid', 'iid', 'label'])
# train_df = pd.DataFrame(train_set, columns = ["rid", 'uid', 'iid', 'label'])
# valid_df = pd.DataFrame(valid_set, columns = ["rid", 'uid', 'iid', 'label'])
# test_df = pd.DataFrame(test_set, columns = ["rid", 'uid', 'iid', 'label'])

click_df = pd.DataFrame(click_list, columns = ["rid", 'uid', 'iid', 'label'])
...
pos_df = pos_df.sort_values(axis = 0, ascending = True, by = 'iid')
